...
def generate_wechat_compatible_html(script_data, image_urls):
    """
    生成微信公众号兼容的HTML
    - 使用table布局替代grid
    - 使用纯色背景替代渐变
    ...
    for i, panel in enumerate(script_data.get("panels", []), 1):
        ...
        if image_urls and i <= len(image_urls) and image_urls[i-1]:
            html_parts.append(f"<img src='{image_urls[i-1]}' style='width: 100%; max-width: 280px; border-radius: 8px; display: block;' />")
...
========================================
    ...
    # 生成兼容HTML
    print("\n生成微信公众号兼容HTML...")
    html_content = generate_wechat_compatible_html(script_data, image_urls)
    print("✓ HTML生成完成")

    # 保存预览
    preview_file = Path("D:/自动化生产动漫工具/comic-generator/output/wechat_compatible_article.html")
    with open(preview_file, 'w', encoding='utf-8') as f:
        f.write("<!DOCTYPE html><html><head><meta charset='UTF-8'><title>微信兼容版本</title></head><body style='background: #f5f5f5; padding: 20px;'>")
        f.write(html_content)
        f.write("</body></html>")
    print(f"✓ 预览已保存: {preview_file}")
    ...
    article = {
        "title": f"{script_data.get('title', 'AI四格漫画')}（微信兼容版）",
        "author": "AI",
        "digest": "悟空和AI小智的爆笑故事",
    ...
    if "media_id" in data:
        ...
        print(f"{'='*70}")
        print(f"\n💡 提示：")
        print(f"1. 在浏览器中打开 {preview_file} 查看预览")
        print(f"2. 登录微信公众号后台查看最新草稿")
        print(f"3. 标题包含'（微信兼容版）'")
        print(f"4. 使用table布局和纯色背景，应该可以正常显示")
    else:
        print(f"❌ 上传失败: {data}")
